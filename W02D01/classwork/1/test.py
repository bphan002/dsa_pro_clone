...
class TestExample(unittest.TestCase):
    ...
    def run_test(self, input_values, expected_value):
        ...
        try:
            self.assertEqual(minSubArrayLen(solution, *input_values), expected_value)
        except TimeoutError:
            self.fail()

    @timeout(2)
    def test_case_1(self):
        self.run_test([7, [2,3,1,2,4,3]], 2)
    
    @timeout(2)
    def test_case_2(self):
        self.run_test([4,[1,4,4]], 1)

    @timeout(2)
    def test_case_3(self):
        self.run_test([11, [1,1,1,1,1,1,1,1]], 0)
